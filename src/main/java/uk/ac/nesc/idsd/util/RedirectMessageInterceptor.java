...
public class RedirectMessageInterceptor extends MethodFilterInterceptor {
    ...
    protected void before(ActionInvocation invocation, ValidationAware validationAware) throws Exception {
        ...
        if (fieldErrors != null && fieldErrors.size() > 0) {
            for (Object entryO : fieldErrors.entrySet()) {

                Map.Entry entry = (Map.Entry) entryO;
//				if(entry.getValue() instanceof java.util.ArrayList) {
//					log.error("This is arraylist");
//					log.error(((ArrayList) ((Map.Entry) entry).getValue()).size());
//				
//				}
                validationAware.addFieldError((String) entry.getKey(), ((ArrayList) entry.getValue()).get(0).toString());
            }
        }
