...
public abstract class AbstractUserDetailsAuthenticationProvider implements AuthenticationProvider, InitializingBean,
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        logger.error("@@ - authenticationToken = " + authentication.getName() + ", " + authentication.getCredentials() + ", " + authentication.getPrincipal());
        // Determine username
        ...
        if (user != null) {
            logger.error("@@ - cached user : " + user.getUsername() + " , " + user.getPassword());
        }

        if (user == null) {
            cacheWasUsed = false;
            try {
                logger.error("@@ - passwrod: " + authentication.getCredentials());
                user = retrieveUser(username, (UsernamePasswordAuthenticationToken) authentication);
                logger.error("@@ - db password: " + user.getPassword());
