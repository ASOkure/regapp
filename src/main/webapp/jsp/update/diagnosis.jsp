...
<script type="text/javascript">
  YUI().use("node", function (Y) {
    Y.on("domready", edit_diagnosis_init);
  })
</script>

<div id="edit-page-div">
  <h1>Update Record: Register ID <s:property value="registerId"/> - Clinical History & Biomaterials</h1>
              ...
              <td>
                ...
                <s:if test="type == 'string'">
                  <s:if test="mandatory == 1">
                    <s:textfield name="%{paramName}" value="" theme="simple" cssClass=''/>
                  </s:if>
                  <s:else>
                    <s:textfield name="%{paramName}" value="" theme="simple"/>
                  </s:else>
                </s:if>
                <s:if test="type == 'year'">
                  <s:textfield name="%{paramName}" theme="simple" cssClass='int-range-1900-9999'/>
                </s:if>
                <s:if test="type == 'menu'">
                  <s:if test="label == 'Age of First Presentation'">
                    <s:select name="%{paramName}" label="%{para.label}" list="menu.options" listKey="value" listValue="value" emptyOption="false" headerKey="" headerValue="--- Please Select ---" theme="simple"/>
                  </s:if>
                  <s:else>
                    <s:select name="%{paramName}" label="%{para.label}" list="menu.options" listKey="value" listValue="value" emptyOption="false" headerKey="" headerValue="--- Please Select ---" theme="simple" cssClass='validate-selection'/>
                  </s:else>
                </s:if>
                <s:if test="type == 'auto_fill'">
                  <s:textfield name="%{paramName}" theme="simple" cssClass='auto-text' readonly="true"/>
                </s:if>
