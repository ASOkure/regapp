...
Validator.messageSource['en-us'] = [
	...
	['decimal-places', 'Please use less than %s decimal places.'],
	...
	['equals', 'Value does not match above.'],
	...
	['validate-time' , 'Please enter a validate time, for example 23:59:59 or 23:59'],
	['validate-password' , 'A validate password should have at least 8 characters, including a lower case letter, a upper case letter, a number, a special character (@#$%^&+=), and no whitespace allowed'],
	['validate-username' , 'A validate username should contains only letters, numbers, "_", and "-", length should be between 3 and 30.']
]
...
Validator.messageSource['zh-cn'] = [
	...
	['validate-chinese','����������'],
	['validate-phone','��������ȷ�ĵ绰����,��:010-29392929,��ǰ����Ϊ%s.'],
...
ValidationUtils = {
	...
	fireSubmit: function(form) {
 	...
 	getMessageSource : function() {
 		...
 		var messageSource = Validator.messageSource['en-us'];
...
Validation.addAllThese([
	...
	['validate-time',/^([0]\d|[1]\d|[2][0-3]):[0-5]\d(:[0-5]\d|)$/], //allow 24H time as: 23:59:59 or 23:59
	['validate-username', /^[a-zA-Z0-9_-]{3,30}$/], //letters (Upper or lower), numbers, _ and -, length between 3 and 30
	['validate-password',/^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=])(?=\S+$).{8,}$/]
]);
...
Validation.addAllThese([
	...
	/*
	 * Usage: decimal-places-$decimalPlaces
	 * Example: 2 = float-range-2
	 */
	['decimal-places', function (v, elm, args, metadata) {
		metadata._error = ValidationUtils.format(ValidationUtils.getI18nMsg(metadata.className));
		return (decimalPlaces(v) <= parseInt(args[0]))
	}, {depends: ['validate-number']}],
	/*
	...
	['validate-date', function(v,elm,args,metadata) {
		...
		metadata._error = ValidationUtils.format(ValidationUtils.getI18nMsg(metadata.className), [dateFormat, dateFormat.replace('yyyy', '2016').replace('MM', '03').replace('dd', '12')]);
	...
	//['validate-phone',/^\+[2]-[2-4]-[3-4]-[3-4]$/],
	//['validate-phone',/^((\+)|(0{2}))?(\d{2,3})-?\s?(\d{3})-?\s?(\d{3})-?\s?(\d{2,3})$/],
	['validate-phone',/^((\+)|(0{2}))?(\d{2}|\d{3})-?\s?(\d{3})-?\s?(\d{3})-?\s?(\d{3,4})$/],
